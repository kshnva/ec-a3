...
def main():
    ...
    if best_cpg is not None:
        # Use the saved CPG genome (if the checkpoint included it)
        def cpg_controller(model, data):
            num_actuators = model.nu
            t = data.time
            freqs = best_cpg[:num_actuators]
            amps = best_cpg[num_actuators:2 * num_actuators]
            phases = best_cpg[2 * num_actuators:3 * num_actuators]
        ...
        print("[INFO] No best CPG in checkpoint — using deterministic default controller.")
