...
POP_SIZE_BODY = 5
POP_SIZE_CPG = 2
GENERATIONS_BODY = 40          # outer loop generations
GENERATIONS_CPG_INNER = 1     # inner loop generations
...
def main():
    ...
    print("\nBest body saved. Decoding to robot graph...")

    # Decode genotype -> probability matrices -> robot graph
    nde_genotype = [best_body[:64], best_body[64:128], best_body[128:]]
    nde = NeuralDevelopmentalEncoding(number_of_modules=NUM_OF_MODULES)
    ...
    try:
        ...
        print(f"[INFO] Saved best robot JSON to {json_path}")
    except Exception as e:
        print(f"[WARN] Could not save robot graph JSON: {e}")

    # Construct MuJoCo spec
    robot = construct_mjspec_from_graph(robot_graph)

    # Make tracker + controller
    tracker = Tracker(mujoco_obj_to_find=mj.mjtObj.mjOBJ_GEOM, name_to_bind="core")
    if best_cpg is not None:
        ctrl = CPGController(best_cpg, tracker=tracker)
        print("[INFO] Using saved best CPG controller.")
    else:
        # fallback: simple oscillatory controller
        def nn_ctrl(model, data):
            num_actuators = model.nu
            t = data.time
            data.ctrl[:] = np.sin(2.0 * np.pi * 0.5 * t + np.arange(num_actuators))
            tracker.update(data)
        ctrl = Controller(controller_callback_function=nn_ctrl, tracker=tracker)
        print("[INFO] No best CPG — using deterministic controller.")

    # Run final simulation / visualization
    print("[INFO] Launching final simulation of best body...")
    # Use the saved best CPG or a zero array if missing
    final_cpg = best_cpg if best_cpg is not None else np.zeros(NUM_OF_MODULES*3)
    final_fitness = experiment(robot_graph, final_cpg, duration=VISUALIZE_DURATION, mode="launcher")
    print(f"[RESULT] Fitness of best robot: {final_fitness:.6f}")
